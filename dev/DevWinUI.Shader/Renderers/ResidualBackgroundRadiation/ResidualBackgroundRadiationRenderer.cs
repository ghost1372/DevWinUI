...
public partial class ResidualBackgroundRadiationRenderer : RendererBase
{
    private PixelShaderEffect<ResidualBackgroundRadiationShader>? _residualEffect;
    private float _timeAccumulator = 0f;

    public override void CreateResources(CanvasAnimatedControl sender, CanvasCreateResourcesEventArgs args)
    {
        Dispose();
        _residualEffect = new PixelShaderEffect<ResidualBackgroundRadiationShader>();
    }

    public override void Update(ICanvasAnimatedControl sender, CanvasAnimatedUpdateEventArgs args)
    {
        if (_residualEffect == null || !isEnabled) return;
    ...
    public override void Draw(ICanvasAnimatedControl sender, CanvasAnimatedDrawEventArgs args)
    {
        ...
        _residualEffect.ConstantBuffer = new ResidualBackgroundRadiationShader(
            _timeAccumulator,
        ...
        ds.DrawImage(_residualEffect);
        ResetTransform(ds, isBreathingEffectEnabled);
    }

    public override void Dispose()
    {
        _residualEffect?.Dispose();
        _residualEffect = null;
    }
}
