...
public partial class TriangleGradientBackgroundRenderer : RendererBase
{
    private PixelShaderEffect<TriangleGradientBackgroundShader>? _triangleEffect;
    private float _timeAccumulator = 0f;

    public override void CreateResources(CanvasAnimatedControl sender, CanvasCreateResourcesEventArgs args)
    {
        Dispose();
        _triangleEffect = new PixelShaderEffect<TriangleGradientBackgroundShader>();
    }

    public override void Update(ICanvasAnimatedControl sender, CanvasAnimatedUpdateEventArgs args)
    {
        if (_triangleEffect == null || !isEnabled) return;
    ...
    public override void Draw(ICanvasAnimatedControl sender, CanvasAnimatedDrawEventArgs args)
    {
        ...
        _triangleEffect.ConstantBuffer = new TriangleGradientBackgroundShader(
            _timeAccumulator,
            new float2(width, height)
        );

        ApplyBreathingTransform(ds, center, isBreathingEffectEnabled);
        ds.DrawImage(_triangleEffect);
        ResetTransform(ds, isBreathingEffectEnabled);
    }

    public override void Dispose()
    {
        _triangleEffect?.Dispose();
        _triangleEffect = null;
    }
}
