...
internal static partial class JsonParser
{
    ...
    {
        var jsonText = new System.IO.StreamReader(file.Path).ReadToEnd();
    ...
    private static void ProcessJsonObject(JsonElement jsonElement, string prefix, List<ParserItem> result)
    {
        if (jsonElement.ValueKind == JsonValueKind.Object)
        {
            ...
            {
                var key = string.IsNullOrEmpty(prefix) ? property.Name : $"{prefix}{Constants.ConstantSeparator}{property.Name}";
