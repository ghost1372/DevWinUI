...
public static class LimitedAccessFeatureTokenGenerator
{
    private static readonly Dictionary<string, string> LimitedAccessFeaturesMap = new Dictionary<string, string>()
    {
    ...
    public static string GenerateTokenFromFeatureId(string featureId)
        => GenerateFeatureToken(featureId, LimitedAccessFeaturesMap[featureId], PackageHelper.GetPackageFamilyName());
    public static string GenerateTokenFromFeatureId(string featureId, string code)
        => GenerateFeatureToken(featureId, code, PackageHelper.GetPackageFamilyName());

    public static string GenerateAttestation(string featureId)
        => $"{PackageHelper.GetPackageFamilyName().Split('_').Last()} has registered their use of {featureId} with Microsoft and agrees to the terms of use.";
